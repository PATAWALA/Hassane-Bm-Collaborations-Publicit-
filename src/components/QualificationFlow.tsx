...
export const QualificationFlow = ({ creator }: { creator: CreatorProfile }) => {
  ...
  return (
    <div className="w-full space-y-6">
      {/* 1. SELECTION TYPE (Boutons côte à côte sur desktop, empilés sur mobile) */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            ...
            className={`py-4 px-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 transition-all cursor-pointer ${
              type === 'Publicité'
                ? 'bg-[#FFE135] text-[#0A1128] ring-4 ring-[#FFE135]/30'
                : 'btn-banana-gradient'
            }`}
          >
      ...
      {/* 2. OPTION BUDGET & VALIDATION INSTANTANÉE */}
      {type && (
        <form onSubmit={handleWhatsApp} className="bg-[#0A1128] p-4 rounded-2xl border border-slate-700 space-y-4 animate-in fade-in duration-300">
          <div>
            ...
            <select
              ...
              className="w-full bg-[#101D42] border border-slate-700 rounded-xl p-3 text-white text-sm focus:outline-none focus:border-[#FFE135]"
            >
              {BUDGETS.map((b) => (
          ...
          <button
            type="submit"
            className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 text-sm shadow-lg active:scale-95 transition-all cursor-pointer"
          >
